...
function Library() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <AdminPageHeader
        title="Photo Library"
        description={loading ? "Opening the shelves…" : "Add images, categories and collections."}
      />
          ...
          <CardContent className="p-5">
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              {categories.map((category) => (
                <span
                  key={category.id}
                  className="flex items-center gap-1.5 rounded-full border border-border bg-muted/50 px-3 py-1 text-[11px] text-foreground"
                >
                  {category.name}
                  <button
                    type="button"
        ...
        <section>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
            {collections.map((collection) => (
              <div
                key={collection.id}
                className="overflow-hidden rounded-[8px] border border-border bg-card shadow-sm"
              >
                <Link
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center gap-1.5 border-t border-border py-1.5 text-[10px] tracking-[0.14em] text-destructive uppercase disabled:opacity-50"
                >
                  <Trash2 className="h-3 w-3" />
